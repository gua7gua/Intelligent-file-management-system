...
本轮页面实现以 `doc/prototype/` 中已经完成的 HTML 原型为第一依据，目标是尽量还原原型，而不是重新设计页面。

- 页面布局、视觉层级、网格比例、卡片组织、表格/详情区关系、流程提示、状态标签和主要文案优先保持原型原貌。
- 实现 Vue 页面时优先搬入原型 `<body>` 内容区结构和页面内样式，再把原型 JavaScript 转成 `<script setup>` 的响应式状态和事件。
- 原型中已有的 CSS 类名、设计令牌和公共组件类优先保留，避免用 Element Plus 重新搭一套不同视觉；Element Plus 只在表单校验、消息提示、日期选择、弹窗等能补足交互质量的地方使用。
- `frontend/CLAUDE.md`、`doc/接口文档.md` 和业务文档用于补齐原型没有细化的字段、API、mock、加载/空数据/错误状态和权限边界，不用于推翻原型的布局与交互结构。
- 如果接口字段比原型更完整，字段补充应自然嵌入原型已有分区；只有业务闭环必须新增的字段才新增 UI，不做额外装饰性重排。
- 如果原型和接口文档存在命名差异，展示层尽量保留原型业务语言，数据字段和请求参数按接口文档命名。
...
- 所有本轮路由可访问，无“待实现”占位。
- 页面布局、视觉层级、文案、交互结构和状态反馈尽量还原对应 HTML 原型；字段补充不得明显改变原型页面的组织方式。
